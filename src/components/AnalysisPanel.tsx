  ...
  return (
    <div className="w-80 bg-[#111] border-l border-gray-800 flex flex-col h-full overflow-hidden">
      ...
      <div className="bg-[#1a1a1a] border-b border-gray-800 p-3">
        ...
        {/* Index Registers */}
        <div className="grid grid-cols-2 gap-2 mb-2">
          <div className="bg-black border border-gray-700 rounded p-1.5 flex justify-between items-center cursor-pointer hover:border-cyan-500 transition-colors"
            onClick={() => editRegister('ix', liveRegisters.ix)}>
            <span className="text-xs font-bold text-cyan-500">IX</span>
            <div className="text-sm font-mono text-gray-300 font-bold tracking-widest">
              {h16val(liveRegisters.ix)}
            </div>
          </div>

          <div className="bg-black border border-gray-700 rounded p-1.5 flex justify-between items-center cursor-pointer hover:border-cyan-500 transition-colors"
            onClick={() => editRegister('iy', liveRegisters.iy)}>
            <span className="text-xs font-bold text-cyan-500">IY</span>
            <div className="text-sm font-mono text-gray-300 font-bold tracking-widest">
              {h16val(liveRegisters.iy)}
            </div>
          </div>
        </div>

        {/* Alternate Registers (Shadow) */}
        <details className="mb-2">
          <summary className="text-[10px] font-bold text-gray-500 cursor-pointer hover:text-gray-400 flex items-center gap-1 py-1">
            <span>▸</span> ALTERNATE REGS (')
          </summary>
          <div className="grid grid-cols-2 gap-2 mt-2">
            <div className="bg-black border border-gray-700 rounded p-1.5 flex justify-between items-center cursor-pointer hover:border-blue-500 transition-colors"
              onClick={() => editPair('a_prime', 'f_prime', (liveRegisters.a_prime << 8) | liveRegisters.f_prime)}>
              <span className="text-xs font-bold text-blue-400">AF'</span>
              <div className="text-sm font-mono text-gray-400 font-bold tracking-widest">
                {h8(liveRegisters.a_prime)}{h8(liveRegisters.f_prime)}
              </div>
            </div>

            <div className="bg-black border border-gray-700 rounded p-1.5 flex justify-between items-center cursor-pointer hover:border-purple-500 transition-colors"
              onClick={() => editPair('h_prime', 'l_prime', (liveRegisters.h_prime << 8) | liveRegisters.l_prime)}>
              <span className="text-xs font-bold text-purple-400">HL'</span>
              <div className="text-sm font-mono text-gray-400 font-bold tracking-widest">
                {h16(liveRegisters.h_prime, liveRegisters.l_prime)}
              </div>
            </div>

            <div className="bg-black border border-gray-700 rounded p-1.5 flex justify-between items-center cursor-pointer hover:border-green-500 transition-colors"
              onClick={() => editPair('b_prime', 'c_prime', (liveRegisters.b_prime << 8) | liveRegisters.c_prime)}>
              <span className="text-xs font-bold text-green-400">BC'</span>
              <div className="text-sm font-mono text-gray-400 font-bold tracking-widest">
                {h16(liveRegisters.b_prime, liveRegisters.c_prime)}
              </div>
            </div>

            <div className="bg-black border border-gray-700 rounded p-1.5 flex justify-between items-center cursor-pointer hover:border-yellow-500 transition-colors"
              onClick={() => editPair('d_prime', 'e_prime', (liveRegisters.d_prime << 8) | liveRegisters.e_prime)}>
              <span className="text-xs font-bold text-yellow-400">DE'</span>
              <div className="text-sm font-mono text-gray-400 font-bold tracking-widest">
                {h16(liveRegisters.d_prime, liveRegisters.e_prime)}
              </div>
            </div>
          </div>
        </details>
        ...
        <div className="grid grid-cols-3 gap-2 mb-2">
          <div className="bg-black border border-gray-700 rounded p-1.5 flex flex-col cursor-pointer hover:border-orange-500 transition-colors"
            onClick={() => editRegister('i', liveRegisters.i)}>
            <span className="text-[10px] font-bold text-orange-500">I</span>
            <div className="text-xs font-mono text-gray-300 font-bold">{h8(liveRegisters.i)}</div>
          </div>

          <div className="bg-black border border-gray-700 rounded p-1.5 flex flex-col cursor-pointer hover:border-orange-500 transition-colors"
            onClick={() => editRegister('r', liveRegisters.r)}>
            <span className="text-[10px] font-bold text-orange-500">R</span>
            <div className="text-xs font-mono text-gray-300 font-bold">{h8(liveRegisters.r)}</div>
          </div>

          <div className="bg-black border border-gray-700 rounded p-1.5 flex flex-col cursor-pointer hover:border-pink-500 transition-colors"
            onClick={() => editRegister('pc', liveRegisters.pc)}>
            <span className="text-[10px] font-bold text-pink-500">PC</span>
            <div className="text-xs font-mono text-gray-300 font-bold">{h16val(liveRegisters.pc)}</div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-[#0d0d0d]">
          <div className="p-4 space-y-1">
            ...
            {stackWords.map((item, idx) => {
              ...
              return (
                <div key={item.addr} className={`flex items-center justify-between p-2 rounded border bg-[#1a1a1a] font-mono text-xs transition-all ${isSp ? 'border-red-500 bg-red-900/10 shadow-[inset_0_0_10px_rgba(239,68,68,0.2)]' : 'border-gray-800'
                  }`}>
