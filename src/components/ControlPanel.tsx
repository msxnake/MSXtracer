...
import { Upload, SkipForward, RotateCcw, FileCode, AlertTriangle, Cpu, CornerDownRight, CornerUpLeft, Layers, Search, X, CheckCircle, XCircle, Undo2, Play, Pause, Edit, Eye, Monitor, Zap, FastForward, HardDrive, Download, Target } from 'lucide-react';
...
interface ControlPanelProps {
  ...
  onToggleTiming: () => void; // NEW: Timing panel toggle
  ...
  return (
    <div className="w-80 bg-[#111] border-r border-gray-800 flex flex-col h-full overflow-y-auto relative">
      ...
      <div className="p-6 flex-1">
        ...
        {/* HALT Mode Indicator */}
        {appState.isHalted && (
          <div className="mb-4 bg-orange-900/30 border border-orange-700 rounded p-3 animate-fade-in">
            <div className="flex items-center gap-2 mb-2">
              <div className="w-2 h-2 rounded-full bg-orange-500 animate-pulse shadow-[0_0_8px_#f97316]"></div>
              <span className="text-xs font-bold text-orange-300 uppercase tracking-wide">CPU HALTED</span>
            </div>
            <div className="space-y-1">
              <p className="text-[10px] text-orange-400">Waiting for interrupt...</p>
              <div className="flex justify-between text-[10px]">
                <span className="text-orange-400">HALT cycles:</span>
                <span className="text-white font-mono font-bold">{appState.haltCyclesAccumulated.toLocaleString()}</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-2 mb-2">
          {appState.analysis && !isFinished ? (
            <>
              <button
                ...
                className={`w-14 flex items-center justify-center rounded transition-all shadow-md ${appState.isPlaying
              ...
              <select
                value={appState.executionSpeed}
                onChange={(e) => onSpeedChange(Number(e.target.value))}
                className="text-xs bg-gray-800 border border-gray-600 text-white rounded px-2 py-1 outline-none focus:border-blue-500"
          ...
          <button
            ...
            className={`flex-1 text-white py-3 rounded flex flex-col items-center justify-center gap-1 transition-all active:scale-95 shadow-md ${isManualMode
        ...
        <div className="flex gap-2 mb-4">
          <button
            ...
            className={`flex-1 py-2 rounded flex items-center justify-center gap-2 border transition-all ${canStepIn
          ...
          <button
            ...
            className={`flex-1 py-2 rounded flex items-center justify-center gap-2 border transition-all ${canStepOut
        ...
        {/* Breakpoint Manager Button */}
        <div className="mb-2">
          <button
            onClick={onToggleBreakpoints}
            className={`w-full border py-2 rounded flex items-center justify-center gap-2 transition-all ${(appState.conditionalBreakpoints.length +
              appState.memoryWatchpoints.length +
              appState.registerWatchpoints.length +
              appState.accessBreakpoints.length) > 0
              ? 'bg-red-900/20 border-red-700 text-red-400 hover:border-red-500'
              : 'bg-[#1a1a1a] border-gray-700 text-gray-400 hover:border-red-500 hover:text-red-400'
              }`}
          >
            <Target size={16} />
            <span className="text-xs font-bold">
              BREAKPOINTS
              {(appState.conditionalBreakpoints.length +
                appState.memoryWatchpoints.length +
                appState.registerWatchpoints.length +
                appState.accessBreakpoints.length) > 0 &&
                ` (${appState.conditionalBreakpoints.length +
                appState.memoryWatchpoints.length +
                appState.registerWatchpoints.length +
                appState.accessBreakpoints.length})`
              }
            </span>
          </button>
        </div>
        ...
        <div className="mb-6">
          <button
            ...
            className={`w-full border py-2 rounded flex items-center justify-center gap-2 transition-all ${appState.watchlist.length > 0
              ? 'bg-cyan-900/20 border-cyan-700 text-cyan-400 hover:border-cyan-500'
          ...
          >
            ...
            <span className="text-xs font-bold">
              WATCHLIST
              {appState.watchlist.length > 0 && ` (${appState.watchlist.length})`}
            </span>
          </button>
        </div>
        ...
        <div className="mb-6">
          <button
            ...
            className={`w-full border py-2 rounded flex items-center justify-center gap-2 transition-all ${appState.cbiosState.loaded
        ...
        {/* Export ROM Button */}
        {appState.analysis && onExportROM && (
          <div className="mb-6">
            <button
              onClick={onExportROM}
              className="w-full bg-cyan-900/30 border border-cyan-700 hover:border-cyan-500 text-cyan-400 hover:text-cyan-300 py-2 rounded flex items-center justify-center gap-2 transition-all"
              title="Export assembled code as MSX ROM file"
            >
              <Download size={16} />
              <span className="text-xs font-bold">EXPORT ROM</span>
            </button>
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            {currentStep && (
              <div className="animate-fade-in">
                <div className={`border-l-4 p-3 rounded bg-opacity-10 ${currentStep.type === 'BUG_WARNING' ? 'border-red-500 bg-red-900' :
                  currentStep.type === 'LOOP' ? 'border-yellow-500 bg-yellow-900' :
                    currentStep.type === 'CALL' ? 'border-purple-500 bg-purple-900' :
                      'border-blue-500 bg-blue-900'
                  }`}>
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${currentStep.type === 'BUG_WARNING' ? 'bg-red-600 text-white' :
        ...
        <div className="absolute inset-0 bg-black/80 backdrop-blur-sm z-50 flex flex-col p-4 animate-fade-in">
          ...
          {searchResult && (
            <div className="flex-1 overflow-auto">
              ...
              <div className={`p-3 rounded border mb-2 ${searchResult.isReachable ? 'bg-green-900/20 border-green-500/50' :
